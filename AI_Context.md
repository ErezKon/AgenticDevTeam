...
| Adaptive-only Anthropic models omit `temperature`/`topK`/`topP` | `llm-provider.ts` | Models matching `ADAPTIVE_ONLY_MODEL_PREFIXES` in `@langchain/anthropic` (`claude-opus-4-7+`, `claude-opus-4-8+`, `claude-opus-5+`, `claude-fable-5+`, `claude-mythos-*`) reject sampling params at call time. The regex must stay in sync with the SDK's `params.ts`. |
